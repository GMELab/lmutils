                ...
                if let Some(mut data) = data {
                    rayon::scope(|s| {
                        s.spawn(|_| {
                            ...
                            let _e = s.enter();
                            let mut tries = 1;
                            while std::panic::catch_unwind(AssertUnwindSafe(|| f(&mut data)))
                                .is_err()
                            {
...
pub fn convert_file(from: &str, to: &str) -> Result<(), crate::Error> {
...
// });
//     Ok(unsafe {
//         std::mem::transmute::<std::vec::Vec<std::mem::MaybeUninit<calc::R2>>, std::vec::Vec<calc::R2>>(
//             results,
//         )
//     })
// }
...
//     Ok(unsafe {
//         std::mem::transmute::<
//             std::vec::Vec<std::mem::MaybeUninit<calc::PValue>>,
//             std::vec::Vec<calc::PValue>,
//         >(results)
//     })
// }
